...
class Mesh2_BiRealNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layers = []
        layers.append(Mesh_Block(block,self.inplanes, planes, stride, downsample))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(Mesh_Block(block,self.inplanes, planes))
...
class En2_BiRealNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        # self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        # self.w1 = channel_w(512)
        # self.w2 = channel_w(512)
        # self.b1 = nn.BatchNorm1d(512,affine=False)
        # self.b2 = nn.BatchNorm1d(512,affine=False)
        self.bn2 = nn.BatchNorm1d(512 * block.expansion*2)
        # self.bn3 = nn.BatchNorm1d(512 * block.expansion)
...
def Meshk2_resnet18(pretrained=False, **kwargs):
    """Constructs a BiRealNet-18 model. """
    model = Mesh2_BiRealNet(BasicBlock, [4, 4, 4, 4], **kwargs)
    return model
def Meshk2_resnet34(pretrained=False, **kwargs):
    """Constructs a BiRealNet-18 model. """
    model = Mesh2_BiRealNet(BasicBlock, [6, 8, 12, 6], **kwargs)
    return model
def EnK2_resnet18(pretrained=False, **kwargs):
    """Constructs a BiRealNet-18 model. """
    model = En2_BiRealNet(BasicBlock, [4,4,4,4], **kwargs)
    return model
