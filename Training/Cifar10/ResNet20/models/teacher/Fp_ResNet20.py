...
from util.binary_modules import BinaryActivation, HardBinaryConv,channel_w
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.binary_conv = nn.Conv2d(inplanes, planes, kernel_size=3, stride=stride, padding=1,bias=False)
...
class Mesh2_block_bn(nn.Module):
    def __init__(self, block, in_ch, out_ch,stride=1,downsample=None):
        super(Mesh2_block_bn, self).__init__()
        self.block1 = block(in_ch, out_ch,stride=stride,downsample=downsample)
        self.block2 = block(in_ch, out_ch,stride=stride,downsample=downsample)
        self.dropout1 = channel_w(out_ch)
        self.dropout2 = channel_w(out_ch)
    ...
    def forward(self,x):
        ...
        out1 = self.bn1(out1 + resx2)
        out2 = self.bn2(out2 + resx1)
...
class Mesh2_BiRealNet_BN(nn.Module):

    def __init__(self, block, layers, num_classes=10, zero_init_residual=False):
        super(Mesh2_BiRealNet_BN, self).__init__()
        self.inplanes = 16
        self.conv1 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1,bias=False)
        self.bn1 = nn.BatchNorm2d(16)
        # self.conv2 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3,
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layers = []
        layers.append(Mesh2_block_bn(block,self.inplanes, planes, stride, downsample))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(Mesh2_block_bn(block,self.inplanes, planes))
...
def MeshNet20_K2_bn(pretrained=False, **kwargs):
    """Constructs a BiRealNet-18 model. """
    model = Mesh2_BiRealNet_BN(BasicBlock, [6,6,6], **kwargs)
    return model
