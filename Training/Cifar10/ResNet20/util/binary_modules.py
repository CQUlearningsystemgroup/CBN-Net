...
class HardBinaryConv(nn.Module):
    def __init__(self, in_chn, out_chn, kernel_size=3, stride=1, padding=1,group=1):
        super(HardBinaryConv, self).__init__()
        self.group = group
        self.stride = stride
        self.padding = padding
        self.number_of_weights = in_chn * out_chn * kernel_size * kernel_size
        self.shape = (out_chn, int(in_chn/group), kernel_size, kernel_size)
    ...
    def forward(self, x):
        ...
        y = F.conv2d(binary_a, binary_weights, stride=self.stride, padding=self.padding,groups=self.group)
